...
#define ERR_CASE(val)              \
    ...
    switch (res) {
        ...
        default:
            cerr << "Unknown SPIRV-Reflect error";
            break;
    }
...
static VkShaderStageFlagBits getStage(string_view name)
{
    ...
    if (suffix == "vert") {
        ...
        cerr << "Invalid shader stage" << endl;
        fatalExit();
    }
}

static vector<uint32_t> compileToSPV(const DynArray<char> &src,
                                     ...
                                     const vector<string> &defines)
{
    ...
    switch (vk_stage) {
        ...
        default: {
            cerr << "Unknown mapping from vulkan stage to glslang" << endl;
            fatalExit();
        }
    }
                                                  ...
                                                  VkShaderStageFlagBits stage)
{
    ...
    DynArray<SpvReflectDescriptorSet *> desc_sets(num_sets);
      ...
      base_pool_sizes_()
{
    ...
    vector<DynArray<VkDescriptorSetLayoutBinding>> binding_infos;
    ...
    for (const auto &desc_set : reflected_sets) {
        DynArray<VkDescriptorSetLayoutBinding> set_binding_info(
    ...
    for (int set_id = 0; set_id < (int)binding_infos.size(); set_id++) {
        const DynArray<VkDescriptorSetLayoutBinding> &set_binding_info =
