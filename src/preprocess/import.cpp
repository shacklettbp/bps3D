...
Material Material::make(const string_view material_name,
                        const string_view albedo_name,
                        const glm::vec3 &base_albedo,
                        float roughness)
{
    return Material {
    ...
    optional<string_view> texture_dir)
{
    ...
    if (isHabitatJSON(scene_path)) {
        return parseHabitatJSON<VertexType, MaterialType>(
            scene_path, base_txfm, texture_dir);
    }
