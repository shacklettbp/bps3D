...
GLTFScene gltfLoad(filesystem::path gltf_path) noexcept
{
    ...
    if (binary) {
        ifstream binary_file(string(gltf_path), ios::in | ios::binary);
    ...
    try {
        ...
        if (textures_elem.error() != simdjson::NO_SUCH_FIELD) {
            for (const auto &texture : textures_elem.get_array()) {
                ...
                if (src_err) {
                    ...
                    if (ext_err) {
                        cerr << "GLTF loading '" << gltf_path
                             << "' failed: texture without source" << endl;
                        abort();
                    }
                }
        ...
        if (scenes.size() > 1) {
            cerr << "GLTF loading '" << gltf_path
                 << "' failed: Multiscene files not supported" << endl;
            abort();
        }
...
static void dumpGLTFTexture(const GLTFScene &scene,
                            const GLTFImage &img,
                            string_view texture_dir,
                            string_view texture_name)
{
    ...
    tex_dump.write(reinterpret_cast<const char *>(tex_ptr),
        ...
        if (tex_idx < scene.textures.size()) {
            ...
            if (img.type == GLTFImageType::EXTERNAL) {
                ...
                if (img.type == GLTFImageType::JPEG) {
                    ...
                    cerr << "GLTF: Unsupported internal image type" << endl;
                    abort();
                }
                ...
                if (texture_dir.has_value()) {
                    ...
                    if (inserted.second) {
                        dumpGLTFTexture(scene, img, texture_dir.value(),
                                        tex_name);
                    }
                }
            }
...
template <typename T>
struct HasPosition<T, decltype((void)T::px, 0)> : std::true_type {};

template <typename T, typename = int>
struct HasNormal : std::false_type {};
template <typename T>
struct HasNormal<T, decltype((void)T::nx, 0)> : std::true_type {};

template <typename T, typename = int>
struct HasUV : std::false_type {};
template <typename T>
struct HasUV<T, decltype((void)T::ux, 0)> : std::true_type {};
    ...
    uint32_t mesh_idx)
{
    ...
    if constexpr (has_uv) {
        if (mesh.uvIdx.has_value()) {
            uv_accessor = getGLTFAccessorView<const glm::vec2>(
                scene, mesh.uvIdx.value());
        }
    }
    ...
    if (index_type == GLTFComponentType::UINT32) {
        ...
        cerr << "GLTF loading failed: unsupported index type" << endl;
        abort();
    }
    ...
    for (uint32_t vert_idx = 0; vert_idx <= max_idx; vert_idx++) {
        ...
        if constexpr (has_position) {
            const auto &pos = (*position_accessor)[vert_idx];
            vert.px = pos.x;
            vert.py = pos.y;
            vert.pz = pos.z;
        }
        ...
        if constexpr (has_normal) {
            if (normal_accessor.has_value()) {
                const auto &normal = (*normal_accessor)[vert_idx];
                vert.nx = normal.x;
                vert.ny = normal.y;
                vert.nz = normal.y;
            }
        ...
        if constexpr (has_uv) {
            if (uv_accessor.has_value()) {
                const auto &uv = (*uv_accessor)[vert_idx];
                vert.ux = uv.x;
                vert.uy = uv.y;
            }
        }
